...
function initmap() {
	...
	try{
        ...
        imageryLayers.addImageryProvider(labelImagery);
     	...
     	/*  document.getElementById("1right").onmouseout=function(){//当鼠标滑出
		...
		index= layer.load(1, {
		  shade: [0.1,'#fff'] //0.1透明度的白色背景
                 ...
                 debugger
     			var mylayer = scene.layers.find("T4@白模_1");
     ...
     /* 	    			 if(mapbuilding.get(entity.name)=="3a412e01d15945f7b386718fb9363dc5"){
     		 ...
     		 /*    			 if(mapbuilding.get(entity.name)=="3a412e01d15945f7b386718fb9363dc5"){
     	    				 ...
     	    				 for(j=0;j<unitsB.length;j++){
         						 ...
         						 html += "<td style='cursor:pointer' onclick=\"test('"+unitsB[j]+"-"+floor[i].floorname+""+roomsA[j]+"')\" onmouseout=\'hideRoom()\' onmouseover=\'showRoom("+entity.name+","+floor[i].floorname+','+(j+1)+")\' id=\""+unitsB[j]+"-"+floor[i].floorname+""+roomsA[j]+"\">"+unitsB[j]+"-"+floor[i].floorname+""+roomsA[j]+"</td>"; 
...
<script>
	...
	var table_sg_html="";
	$("#table_sg").empty();
	table_sg_html+=" <thead><tr><th>序号</th><th>名称</th><th>进度状态</th><th style='cursor:pointer;' onclick='flytofirst()'>恢复</th></tr></thead>";
	for(i=0;i<mainData.BUILDING.length;i++){
		table_sg_html+="	<tr class='text-center'><td>0"+(i+1)+"</td><td>"+mainData.BUILDING[i].NAME+"楼</td><td><span class="+mainData.BUILDING[i].SGSTATE+"></span></td><td><img style='cursor:pointer' onclick='fly("+i+")'  src='../../../resources/imgs/bz.png'></td></tr>";
		
	}
	$("#table_sg").append(table_sg_html);
    var table_sg_html2="";
	$("#table_sg2").empty();
	table_sg_html2+=" <thead><tr><th>序号</th><th>名称</th><th>进度状态</th><th style='cursor:pointer;' onclick='flytofirst()'>恢复</th></tr></thead>";
	for(i=0;i<mainData.BUILDING.length;i++){
		table_sg_html2+="	<tr class='text-center'><td>0"+(i+1)+"</td><td>"+mainData.BUILDING[i].NAME+"楼</td><td><span class="+mainData.BUILDING[i].SGSTATE2+"></span></td><td><img style='cursor:pointer' onclick='fly("+i+")'  src='../../../resources/imgs/bz.png'></td></tr>";
...
function changeStyle(i){
	...
	var layer5 = scene.layers.find("T4@白模_1");
...
var layer = scene.layers.find("T4@白模_1");
			layer.removeAllObjsColor();
...
function test(a){
	...
	//layer.removeAllObjsColor();
	if(a=='1-701'){
	 layer.setObjsColor([12],Cesium.Color.BLUE);
	}else if(a=='1-702'){
	layer.setObjsColor([232],Cesium.Color.BLUE);
	layer.setObjsColor([12],Cesium.Color.YELLOW);
	layer.setObjsColor([174],Cesium.Color.YELLOW);
	}else if(a=='2-703'){
	layer.setObjsColor([12],Cesium.Color.YELLOW);
	layer.setObjsColor([232],Cesium.Color.MAGENTA);
	layer.setObjsColor([174],Cesium.Color.BLUE);
	}else if(a=='2-704'){
	layer.setObjsColor([194],Cesium.Color.BLUE);
	}
}
